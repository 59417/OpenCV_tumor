...
# 轉換成JPG
# 指定.dcm文件夾路徑
folder_path = '/Users/Downloads/project-OpenCV-tumor/Raw'
# 指定輸出jpg/png文件夾路徑
jpg_folder_path = '/Users/Downloads/project-OpenCV-tumor/convert'
emptydir(jpg_folder_path)
images_folder_path = os.listdir(folder_path)
images_folder_path.remove('.DS_Store')
...
for i in images_folder_path:
    ...
    for image in images_path:
        ...
        # 讀取檔案是 RGB > 轉成 opencv 的 BGR
        cv2.imwrite(os.path.join(jpg_folder_path +"/" + i, image), pixel_array_numpy) # 存檔
        
'''
(base) User@MBP project-OpenCV-tumor % ls
OpenCV偵測腫瘤.pptx	Raw			notes
OpenCV偵測腫瘤.py	convert
(base) User@MBP project-OpenCV-tumor % cd convert 
(base) User@MBP convert % ls
ESO-051(O)	ESO-052(O)	ESO-053(O)
'''
...
import os
import glob  # 找檔案
path = (glob.glob("/Users/Downloads/project-OpenCV-tumor/convert/ESO-052(O)/*.jpg"))
path
...
# 只取第一個病患所有照片做ROI裁切
if checked == 1:
    output_folder = path[0][0:55] + 'ROI/'+ path[0][63:74]
    if not os.path.exists(output_folder): os.makedirs(output_folder)
    # 資料夾中所有圖片自動裁切
    for i in path:
        image_read = cv2.imread(i)
        roiImg = image_read[y: y + h, x:x + w]
        cv2.imwrite(output_folder + i[74:], roiImg)
else:
    print("program shutdown")
    
'''
(base) User@MBP % cd /Users/Downloads/project-OpenCV-tumor/ROI
(base) User@MBP ROI % cd ESO-052/(O) 
(base) User@MBP ESO-052(O) % ls
I10.jpg	I270.jpg I440.jpg I610.jpg I790.jpg
I100.jpg I280.jpg I450.jpg I620.jpg I80.jpg
I110.jpg	 I290.jpg I460.jpg I630.jpg I800.jpg
...
'''
...
# 將亮點標記
# 讀取圖片路徑
path = (glob.glob("/Users/Downloads/project-OpenCV-tumor/ROI/ESO-052(O)/*.jpg")) # 匯入所有jpg圖片路徑
print(path)
output_folder = path[0][0:55] + 'bright spot/' + path[0][59:70] # 建立一個輸出資料夾名稱
if not os.path.exists(output_folder): os.makedirs(output_folder) # 如果目錄下無此資料夾就創建一個
# 迴圈一筆筆讀取
# i每跑一次就收到一張圖片路徑
for i in path:
    ...
    if maxVal >= 175:
        ...
        cv2.putText(img, str(i[70:]), (400,300), 
                    cv2.FONT_HERSHEY_SIMPLEX, 1,
                    (50, 168, 82), 2, cv2.LINE_AA) 
        #cv2.putText(影像, 文字, 座標, 字型, 大小, 顏色, 線條寬度, 線條種類)
        cv2.imwrite(output_folder + i[70:], img) # 儲存亮點照片
        cv2.imshow("results", img) # 秀出圖片
        cv2.waitKey(1000) # 圖片顯示持續時間(單位毫秒)，0為一直顯示
        cv2.destroyAllWindows()
    else:
        # 儲存沒有亮點的照片
        cv2.imwrite(output_folder + i[70:], img)

#%%
# 二值化圖片以求得腫瘤亮點面積
os.chdir('/Users/Downloads/project-OpenCV-tumor/')
...
# 以 I670.jpg 為例
image = cv2.imread('./ROI/ESO-052(O)/I670.jpg')
gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) # 將圖片轉成灰階
cv2.imshow("gray", gray)
blur = cv2.GaussianBlur(gray, (41,41),0) # 高斯模糊
cv2.imshow("blur", blur)
thresh = cv2.threshold(blur, 175, 255, cv2.THRESH_BINARY)[1] # 腫瘤與背景分離的二值化處理
...
f = open("area.txt", "w") # 創建txt準備儲存面積數值
...
for i in path:
    ...
    # cv2.RETR_EXTERNAL外部輪廓；CHAIN_APPROX_SIMPLE簡單輪廓
    print(contours)
    # findContours 查找輪廓  
